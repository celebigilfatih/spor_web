...
<div class="shadcn-page-container">
    ...
    <div class="shadcn-page-header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="shadcn-page-title">Rekorlar Yönetimi</h1>
                <p class="shadcn-page-description">Kulübün rekorlarını yönetin</p>
            </div>
            <a href="' . BASE_URL . '/admin/achievements/create-record" class="shadcn-btn shadcn-btn-primary">
    ...
    <div class="shadcn-card">
        <div class="shadcn-card-header">
            <h3 class="shadcn-card-title">Rekorlar</h3>
            <p class="shadcn-card-description">Tüm rekorlar</p>
        </div>
                ...
                <table class="shadcn-table">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 15%;">Rekor Sahibi</th>
                            <th style="width: 15%;">Değer</th>
    ...
    if (isset($records) && is_array($records) && count($records) > 0) {
        foreach ($records as $record) {
            $statusClass = $record['status'] === 'active' ? 'shadcn-badge-success' : 'shadcn-badge-secondary';
            $statusText = $record['status'] === 'active' ? 'Aktif' : 'Pasif';
            
            $content .= '
                        <tr>
                            <td>' . $record['id'] . '</td>
                            <td><strong>' . htmlspecialchars($record['title']) . '</strong></td>
                            <td>' . htmlspecialchars($record['holder']) . '</td>
                            <td class="text-center">' . htmlspecialchars($record['value']) . '</td>
                            ...
                            <td>
                                <div class="flex items-center gap-2">
                                    <a href="' . BASE_URL . '/admin/achievements/edit-record/' . $record['id'] . '" 
                                    ...
                                    <button type="button" 
                                            onclick="deleteRecord(' . $record['id'] . ', \'' . htmlspecialchars($record['title']) . '\')" 
        ...
        $content .= '
                        <tr>
                            <td colspan="8" class="text-center py-8 text-zinc-500">
                                ...
                                <p class="text-lg font-medium">Henüz rekor eklenmemiş</p>
                                <p class="text-sm mt-2">Yeni rekor eklemek için yukarıdaki butonu kullanın</p>
                            </td>
                        </tr>';
    }
...
<div id="deleteModal" class="shadcn-modal" style="display: none;">
    ...
    <div class="shadcn-modal-content">
        <div class="shadcn-modal-header">
            <h3 class="shadcn-modal-title">Rekoru Sil</h3>
        ...
        <div class="shadcn-modal-body">
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-zinc-600"><strong id="deleteRecordName"></strong> rekorunu silmek istediğinizden emin misiniz?</p>
                </div>
            </div>
        </div>
...
function deleteRecord(id, name) {
    ...
    document.getElementById("deleteForm").action = "' . BASE_URL . '/admin/achievements/delete-record/" + id;
